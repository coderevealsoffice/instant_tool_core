...
export function MergePdf() {
  ...
  const handleFiles = (newFiles: File[]) => {
    if (newFiles.length > 0) {
      setContext("pdf-tools", "merge-pdf")
      addFiles(newFiles)
      router.push("/workspace?tool=merge-pdf&category=pdf-tools")
    }
  }
  ...
  return (
    ...
    >
      ...
      <p className="text-slate-500 dark:text-slate-400 text-sm mb-4">or drop PDFs here</p>
    </div>
  )
}
