...
export function CompressPdf() {
  ...
  const handleFiles = (newFiles: File[]) => {
    if (newFiles.length > 0) {
      setContext("pdf-tools", "compress-pdf")
      addFiles(newFiles)
      router.push("/workspace?tool=compress-pdf&category=pdf-tools")
    }
  }
  ...
  return (
    ...
    >
      ...
      <p className="text-slate-500 dark:text-slate-400 text-sm mb-4">or drop PDF here</p>
    </div>
  )
}
