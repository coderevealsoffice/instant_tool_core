...
import { FileCode2, Play, Copy, Trash2, CheckCircle, Loader2 } from "lucide-react"
import { toast } from "sonner"

export function MinifyJsCanvas() {
  ...
  const handleProcess = async () => {
    ...
    try {
      ...
      const res = await fetch("/api/dev-tools", {
        ...
        body: JSON.stringify({ action: "minify-js", code: inputCode })
      })
    ...
    <div className="max-w-6xl mx-auto space-y-6">
      <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-200">
        <div className="flex items-center gap-3 mb-4">
          ...
          <div>
            <h2 className="text-lg font-bold text-slate-900">JS Minifier</h2>
            <p className="text-sm text-slate-500">Compress your JavaScript code to reduce file size and improve load times.</p>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div className="space-y-3">
          <div className="flex items-center justify-between">
            <label className="font-semibold text-slate-700">Input JavaScript</label>
          ...
          <textarea
            ...
            className="w-full h-[500px] font-mono text-sm p-4 rounded-xl border-slate-200 focus:ring-yellow-500 focus:border-yellow-500 shadow-sm"
      ...
      <div className="flex justify-center">
        <button
          ...
          className="px-8 py-3 bg-yellow-500 hover:bg-yellow-600 text-white font-bold rounded-xl shadow-md transition-all flex items-center gap-2 disabled:opacity-50"
        >
